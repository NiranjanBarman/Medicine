// src/components/PrintInvoice.jsx
...
const amountInWords = (amount) => {
    ...
    return `${numberToWords(rupees)} Rupees${paise ? ` and ${numberToWords(paise)} Paise` : ''} Only`;
};
...
const PrintInvoice = () => {
    ...
    const sale = getCounterSaleById(id);
    ...
    const handleBackToSaleList = () => {
        navigate('/view-counter-sales');
    };

    if (!sale) return <div className="p-4 text-center">Loading bill details...</div>;

    const { billNo, doctorName, patientName, customerName,paymentAmount, address, items = [], totalAmount = 0, saleDate } = sale;
    const orderId = `ORD-${Math.floor(1000000000 + Math.random() * 9000000000)}`;
    const dueAmount = (sale.totalAmount - paymentAmount) > 0 ? (sale.totalAmount - paymentAmount) : 0;
    ...
    return (
        <div className="bg-white min-h-screen">
             <div className="fixed top-4 right-4 z-50 print:hidden flex space-x-2">
                <button onClick={handleBackToSale} className="px-4 py-2 bg-gray-500 text-white rounded-lg shadow-lg hover:bg-gray-600 transition-colors duration-200">
                    Back to Sale
                </button>
                <button onClick={handleBackToSaleList} className="px-4 py-2 bg-gray-500 text-white rounded-lg shadow-lg hover:bg-gray-600 transition-colors duration-200">
                    Back to Sale List
                </button>
                <button onClick={() => window.print()} className="px-4 py-2 bg-blue-600 text-white rounded-lg shadow-lg hover:bg-blue-700 transition-colors duration-200">
                    Print
                </button>
            </div>

            <div id="invoice-print-area" className="w-full max-w-2xl mx-auto p-4 print:shadow-none bg-white text-xs">
                {/* Header reduced in size */}
                <header className="flex justify-between mb-1">
                    <div>
                        <h1 className="text-xl font-bold">Tax Invoice(Counter)</h1>
                ...
                <div className="grid grid-cols-3 gap-4 text-xs">
                    ...
                    <div className="text-center">
                        ...
                        <p>Total Discount Amount: ₹{formatAmount(totalDiscountAmount)}</p>
                    </div>
                    <div className="text-right">
                        {/* <p>MRP Total: ₹{formatAmount(totalMrp)}</p>
                        <p>Total Discount Amount: ₹{formatAmount(totalDiscountAmount)}</p> */}
                        <p>Total Payable Amount: ₹{formatAmount(totalAmount)}</p>
                        <p>Total Due Amount: ₹{formatAmount(dueAmount)}</p>
                        <p>Total Paid Amount: ₹{formatAmount(paymentAmount)}</p>
                    </div>
                </div>

                <div className="mt-2 pt-2 border-t">
                    <p className="font-semibold">Amount In Words: {amountInWords(totalAmount)}</p>
                </div>

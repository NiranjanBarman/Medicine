...
let currentProductCategories = ['TABLET', 'SYRUP', 'CAPSULE', 'INJECTION', 'N/A'];
const strengths = ['Select Strength', '2 MG+40 MG+100 MG', '500 MG', '10 MG', '1G', '250MG', '100 ML', 'N/A'];
const cfUnits = ['AMPLE', 'BOTTLE', 'BOX', 'CAPSULE', 'DROP', 'GM.', 'JAR', 'KG.', 'LTR.', 'ML.', 'NOS.', 'PACKET', 'PAIR', 'PFS', 'PHYL', 'PIECE'];
...
const handleBulkUpload = (e) => {
    ...
    reader.onload = (event) => {
        ...
        const validItems = json.map((row) => ({
            ...
            saleUnit: '',
            // purchaseMrpStrip: '',
            expiryCheckFormat: 'MONTH', // Default for bulk upload if not specified
            onlineItem: false,
            popularItem: false,
            returnable: false,
            status: 'ACTIVE',
    ...
    const generateItemCode = () => {
        ...
        if (month < 3) { // Months 0-2 are Jan-Mar, so previous financial year
            financialYearStart--;
            financialYearEnd = year % 100;
        }

        const uniquePart = crypto.randomUUID().replace(/-/g, '').substring(0, 10);
        return `ITM${uniquePart}_${String(financialYearStart).slice(2)}-${String(financialYearEnd).padStart(2, '0')}`;
    };
    ...
    useEffect(() => {
        if (isEditing && items.length > 0) {
            ...
            if (itemToEdit) {
                ...
                if (itemToEdit.productCategory && !currentProductCategories.includes(itemToEdit.productCategory)) {
    ...
    const handleSubmit = (e) => {
        ...
        if (!productType || !brandName || !tradeName || !manufacturerMake || !hsnCode || !gstIgst || !cfUnit || !purchaseUnit || !saleUnit) {
            alert('Please fill in all required fields (marked with *).');
            return;
        }

        if (productCategory && !currentProductCategories.includes(productCategory)) {
    ...
    const checkboxClass = 'h-4 w-4 text-teal-600 focus:ring-teal-500 border-gray-300 rounded';
    ...
    return (
        <div className="bg-white p-4 rounded-lg shadow-md max-w-full mx-auto border border-gray-200 text-gray-900">
            ...
            <h2 className="text-2xl font-bold text-teal-800">ADD/MODIFY ITEMS / MEDICINES</h2>
            ...
            <form onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4 mb-6">
                    {/* Product Type */}
                    <div>
                        <label htmlFor="productType" className={labelClass}>Product Type {requiredSpan}</label>
                        <select
                            id="productType"
                            value={productType}
                    ...
                    {/* Sub Sub Category */}
                    <div>
                        <label htmlFor="subSubCategory" className={labelClass}>Sub Sub Category</label>
                        <input
                            type="text"
                            id="subSubCategory"
                        ...
                        <div className="flex items-center">
                            ...
                            >
                                ...
                                {generics.map(generic => (
                                    <option key={generic.id} value={generic.id}>{generic.generic1}</option>
                                ))}
                            </select>
                            <button
                                type="button"
                    ...
                    <div className="col-span-2">
                        <label htmlFor="brandName" className={labelClass}>Brand Name {requiredSpan}</label>
                        <input
                            type="text"
                            id="brandName"
                            value={brandName}
                    ...
                    {/* Trade Name */}
                    <div className="col-span-2">
                        <label htmlFor="tradeName" className={labelClass}>Trade Name {requiredSpan}</label>
                        <input
                            type="text"
                            id="tradeName"
                            value={tradeName}
                    ...
                    <div className="col-span-2">
                        <label htmlFor="manufacturerMake" className={labelClass}>Manufacturer/Make {requiredSpan}</label>
                        <div className="flex items-center">
                            <select
                                id="manufacturerMake"
                    ...
                    {/* HSN Code */}
                    <div>
                        <label htmlFor="hsnCode" className={labelClass}>HSN Code {requiredSpan}</label>
                        <input
                            type="text"
                            id="hsnCode"
                            value={hsnCode}
                    ...
                    {/* GST / IGST (%) */}
                    <div>
                        <label htmlFor="gstIgst" className={labelClass}>GST / IGST (%) {requiredSpan}</label>
                        <input
                            type="number"
                            id="gstIgst"
                            value={gstIgst}
                    ...
                    {/* CGST (%) */}
                    <div>
                        <label htmlFor="cgst" className={labelClass}>CGST (%)</label>
                        <input
                            type="number"
                            id="cgst"
                            value={cgst}
                    ...
                    {/* SGST (%) */}
                    <div>
                        <label htmlFor="sgst" className={labelClass}>SGST (%)</label>
                        <input
                            type="number"
                            id="sgst"
                            value={sgst}
                    ...
                    {/* C.F. Unit */}
                    <div>
                        <label htmlFor="cfUnit" className={labelClass}>C.F. Unit {requiredSpan}</label>
                        <select
                            id="cfUnit"
                            value={cfUnit}
                    ...
                    {/* Stock Alert Qty. */}
                    <div>
                        <label htmlFor="stockAlertQty" className={labelClass}>Stock Alert Qty.</label>
                        <input
                            type="number"
                            id="stockAlertQty"
                            value={stockAlertQty}
                    ...
                    {/* Purchase Rate/Strip */}
                    <div>
                        <label htmlFor="purchaseRateStrip" className={labelClass}>Purchase Rate/Strip</label>
                        <input
                            type="number"
                    ...
                    {/* Purchase Unit */}
                    <div>
                        <label htmlFor="purchaseUnit" className={labelClass}>Purchase Unit {requiredSpan}</label>
                        <select
                            id="purchaseUnit"
                            value={purchaseUnit}
                    ...
                    {/* Sale Rate/Strip */}
                    <div>
                        <label htmlFor="saleRateStrip" className={labelClass}>Sale Rate/Strip</label>
                        <input
                            type="number"
                            id="saleRateStrip"
                            value={saleRateStrip}
                    ...
                    {/* Sale Unit */}
                    <div>
                        <label htmlFor="saleUnit" className={labelClass}>Sale Unit {requiredSpan}</label>
                        <select
                            id="saleUnit"
                            value={saleUnit}
                    ...
                    {/* Purchase MRP/Strip */}
                    <div>
                        <label htmlFor="purchaseMrpStrip" className={labelClass}>Purchase MRP/Strip</label>
                        <input
                            type="number"
                            id="purchaseMrpStrip"
                    ...
                    <div className="col-span-full grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 items-end mt-4 pt-4 border-t border-gray-200">
                        ...
                        <div>
                            <label htmlFor="expiryCheck" className={labelClass}>Expiry Check</label>
                            <div className="flex">
                                <input
                                    type="text"
                                    id="expiryCheck"
                                    ...
                                    placeholder={expiryCheckFormat === 'MONTH' ? 'MM/YYYY' : expiryCheckFormat === 'YEAR' ? 'YYYY' : 'DD/MM/YYYY'}
                                />
                                <select
                    ...
                    <div className="col-span-full grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 mt-4">
                        <div className="flex items-center">
                            ...
                            />
                            <label htmlFor="onlineItem" className="ml-2 text-sm text-gray-700">Online Item</label>
                        </div>
                        <div className="flex items-center">
                            <input
                                type="checkbox"
                                ...
                                className={checkboxClass}
                            />
                            <label htmlFor="popularItem" className="ml-2 text-sm text-gray-700">Popular Item</label>
                        </div>
                        <div className="flex items-center">
                            <input
                                type="checkbox"
                                ...
                                className={checkboxClass}
                            />
                            <label htmlFor="returnable" className="ml-2 text-sm text-gray-700">Returnable</label>
                        </div>
                    </div>
                </div>

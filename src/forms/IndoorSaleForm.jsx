...
const IndoorSaleForm = () => {
  ...
  const [admittedPatients, setAdmittedPatients] = useState(localStorage.getItem("userData") ? JSON.parse(localStorage.getItem("userData")) : null);
  ...
  useEffect(() => {
    if (saleId) {
      ...
      if (sale) {
        ...
        // but we keep it for context if the original sale stored an ID that matches one of the admittedPatients.regId
        const patientMatch = admittedPatients.find(p => p.patients?.regId === sale?.patientId);
  ...
  useEffect(() => {
    if (!isUpdatingPayment && selectedPatientAdmissionId) {
      ...
      const admissionRecord = admittedPatients?.find(
